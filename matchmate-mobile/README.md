...
- `api/card.ts`：创建房间、加入房间、房间详情、转账、平摊、撤销、历史和排名。
...
- `useCardWebSocket.ts`：打牌房间事件，如新记账、新平摊、撤销审批、房间成员变化。
...
- 入口页支持创建房间、加入房间、查看活跃房间、历史房间和排名。
- 房间页展示成员、总分、最近局数、资金平摊和撤销状态。
- 记账操作支持转账和资金平摊两种形式。
- 房间内通过 WebSocket 接收实时变更。
