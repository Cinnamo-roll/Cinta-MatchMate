...
public class GlobalExceptionHandler {
    ...
    public BaseResponse handleHttpMessageNotReadable(HttpMessageNotReadableException e) {
        ...
        return ResultUtils.error(ErrorCode.PARAM_ERROR, "请求体格式错误，请检查JSON格式", "");
    }

    @ExceptionHandler(MissingServletRequestParameterException.class)
    public BaseResponse handleMissingParam(MissingServletRequestParameterException e) {
        log.error("missingParam: " + e.getMessage());
        return ResultUtils.error(ErrorCode.PARAM_ERROR, "缺少必要参数: " + e.getParameterName(), "");
    }
}